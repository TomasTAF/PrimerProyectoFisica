...
2. El indice de refraccion del aire n_{2} redondeado a 1
...
http://latex.codecogs.com/gif.latex?sin(\theta_{c})=\frac{n_{2}}{n_{1}}

Podemos observar en el diagrama, la relación geometrica 
http://latex.codecogs.com/gif.latex?sin(\alpha)=\frac{R}{(R+d)}
...
http://latex.codecogs.com/gif.latex?\alpha<\theta{c}

De este modo:
http://latex.codecogs.com/gif.latex?sin(\theta_{c})>\frac{1}{n}
http://latex.codecogs.com/gif.latex?\frac{R}{(R+d)}\geq\frac{1}{n} 

Por lo tanto 
http://latex.codecogs.com/gif.latex?(\frac{R}{(d)})_{min}=\frac{1}{n-1} 
